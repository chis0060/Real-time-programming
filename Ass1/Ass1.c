...
void * readerThread(void *reader) {
	...
	while(monitor.available) {
		...
		/* If the reader reads a pressure thats 80psi or higher */
		if(80 <= (psi = get_pressure(pressure))) {
			printf("Reader %d read %d psi\n",pressure->id+1, psi);
			/* catch the index of the thread in array, so it can be associative */
			monitor.release_id = pressure->id;
			/* grab the value of the pressure */
			monitor.pressure_reading = psi;
			/* set flag indicating a pressure release is required */
			monitor.release = 1;
		}
...
void sigint_handler(int sig) {
	/* Turn off available flag, indicating that the system is no longer online and will
